...
var App = (function() {
  ...
  async function init() {
    ...
    if (!user) {
      Auth.renderModal();
      // Store the post-login init function
      window._appInitAfterAuth = function() { init(); };
      return;
    }
    ...
    // Sync or create current user profile, then block banned users
    await Auth.ensureProfile();
    if (await Auth.isBanned(true)) {
      App.toast('该账号已被管理员封禁，无法登录');
      await Auth.signOut();
      return;
    }
    ...
    // Update UI
    _updateHomeStats();
    document.getElementById('headerTitle').textContent = '刷题助手-浙江600题';
